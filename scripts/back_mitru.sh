...
sqlfile=$backupfolder/mitru-database-$(date +%d-%m-%Y_%H-%M-%S).sql
zipfile=$backupfolder/mitru-database-$(date +%d-%m-%Y_%H-%M-%S).zip
# Create a backup
sudo mysqldump -u $user -p$password mitru > $sqlfile
if [ $? == 0 ]; then
